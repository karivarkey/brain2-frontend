...
export default function Chat() {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (!conv_id) {
        ...
        setSessions((prev) => [
          { id: currentConvId, snippet: userMessage.content.slice(0, 30) },
          ...prev,
        ]);
      }
    } catch (err) {
  ...
  return (
    <div className="flex h-screen bg-white relative w-full overflow-hidden">
      ...
      {/* Sidebar */}
      <div
        className={`fixed sm:relative z-50 h-full flex-shrink-0 transition-all duration-300 flex flex-col max-w-full ${
          isSidebarOpen
            ? "w-64 translate-x-0 border-r"
            : "w-0 -translate-x-full sm:translate-x-0 border-none opacity-0"
        } overflow-hidden`}
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto py-2">
          {isSidebarOpen ? (
            <div className="px-3">
              <p className="text-xs font-medium opacity-50 px-2 mb-2 mt-4">
                Recent
              </p>
              {sessions.map((session) => (
                ...
                >
                  <MessageSquare
                    className={`inline w-4 h-4 mr-2 ${conv_id === session.id ? "opacity-100" : "opacity-50"}`}
                  />
                  {session.snippet}
                </button>
              ))}
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden relative">
        ...
        <div
          className="h-14 flex items-center justify-between px-4 border-b shrink-0 bg-white/80 backdrop-blur-sm z-10"
          style={{ borderColor: colors.border }}
        >
          <button
        ...
        <div className="flex-1 overflow-y-auto w-full px-4 sm:px-12 py-8 pb-32">
          {messages.length === 0 && !loading && (
            <div className="h-full flex flex-col items-center justify-center opacity-40 text-center max-w-sm mx-auto">
              ...
              <p className="text-sm">
                Enter a request to sync your events, fetch memories, or ask a
                question.
              </p>
            </div>
          )}
          ...
          <div className="max-w-3xl mx-auto flex flex-col gap-6">
                ...
                {msg.role === "assistant" &&
                  ...
                  msg.memoryChanges.length > 0 && (
                    <details className="mt-2 w-full max-w-sm group">
                      ...
                      <div
                        className="mt-2 text-xs border rounded-xl p-3 bg-white space-y-2 shadow-sm"
                        style={{ borderColor: colors.border }}
                      >
                        {msg.memoryChanges.map((change, i) => (
                          ...
                          >
                            <div className="flex items-center gap-2">
                              <span className="px-1.5 py-0.5 rounded bg-black/5 font-semibold capitalize text-[10px]">
                                {change.action}
                              </span>
            ...
            {loading && (
              <div
                className="self-start max-w-[85%] flex items-center gap-2 px-5 py-4 rounded-2xl bg-[#f4f4f4] border shadow-sm rounded-bl-sm"
                style={{ borderColor: colors.border }}
              >
                <div
